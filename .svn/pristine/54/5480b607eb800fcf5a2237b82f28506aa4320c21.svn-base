...
class AuthItemController extends RController {
    ...
    public function actionAddPermissions() {
        ...
        if ($childSelectOptions !== array()) {
            ...
            if (isset($_POST['AuthChildForm']) === true) {
                ...
                if ($childFormModel->validate() === true) {
                    // Add the child and load it
                    foreach ($childFormModel->itemname as $key => $value) {
                        $this->_authorizer->authManager->addItemChild($itemName, $value);
                        $child = $this->_authorizer->authManager->getAuthItem($value);
                        $child = $this->_authorizer->attachAuthItemBehavior($child);
                    }
                    // Set a flash message for adding the child
                    Yii::app()->user->setFlash($this->module->flashSuccessKey, Rights::t('core', 'Child :name added.', array(':name' => $child->getNameText()))
                    ...
                    // Reidrect to the same page
                    $this->redirect(array('authItem/addPermissions', 'name' => urlencode($itemName)));
                }
            }
        } else {
            $childFormModel = null;
        ...
        // Render the view
        $this->render('_permissions', array(
            'model' => $model,
            'childFormModel' => $childFormModel,
            'childSelectOptions' => $childSelectOptions,
            'parentDataProvider' => $parentDataProvider,
    ...
    public function actionConfirmPermission() {
        if (isset($_POST['cofirmPermission'])) {
            if ($_POST['cofirmPermission'] == '') {
                echo CJSON::encode(array('error' => 0));
                exit();
            } else if ($_POST['cofirmPermission'] == 'coder') {
                echo CJSON::encode(array('complete' => 2));
                exit();
            } else {
        ...
        $this->renderPartial('_confirmPermission', array(), false, true);
    }
