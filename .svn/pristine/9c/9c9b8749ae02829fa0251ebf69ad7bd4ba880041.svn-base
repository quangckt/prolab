...
<div class="row-fluid">
    <div class="span4">
        <h1 class="title">Duyệt ngày nghỉ của lịch trực</h1>
    </div>
    <div class="span4">
...
$buttonForm = CHtml::ajaxSubmitButton("Cập nhật", Yii::app()->controller->createUrl("submitDayOf"),
    array(
        "dataType" => "json",
        "beforeSend" => "function(){
        ...
        "success" => "function(data){
            if(data.result == true){
                ...
                bootbox.alert('Duyệt ngày nghỉ thành công.');
                $.fn.yiiGridView.update('hr-shift-detail-grid', {
                    data: $(this).serialize()
	            });
            } else {
...
<?php $this->widget('bootstrap.widgets.TbGridView', array(
    ...
    'columns' => array(
        ...
        array(
            'header' => 'Duyệt ngày nghỉ',
            'type' => 'raw',
            'value' => 'CHtml::checkBox("dayOf[".$data->id."]", $data->type, array(""))',
            'htmlOptions' => array(
        ...
        array(
            ...
            'name' => 'noteDayoff',
            'type' => 'raw',
            'value' => 'CHtml::textField("noteDayoff[".$data->id."]", $data->noteDayoff,array("class"=>"span","style" => "margin-bottom:0px;"))',
            'filter' => false,
            'htmlOptions' => array(
                'style' => 'width:300px;'
            ),
