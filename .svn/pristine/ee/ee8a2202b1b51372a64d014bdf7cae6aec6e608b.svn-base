        ...
        if ($assignSelectOptions !== array()) {
            ...
            if (isset($_POST['AssignmentForm']) === true) {
                ...
                if ($formModel->validate() === true) {
                    // Update and redirect
                    foreach ($formModel->itemname as $key => $value) {
                        $this->_authorizer->authManager->assign($value, $model->getId());
                        $item = $this->_authorizer->authManager->getAuthItem($value);
