...
<h1 class="title">Xem phiếu chuyển  <?php echo $model->num; ?></h1>
<hr />
<div class="tools-bar">
<?php 
...
<?php $this->widget('bootstrap.widgets.TbDetailView',array(
	...
	'attributes'=>array(
		...
		'date',
		'description',
        array('name'=>'status','value'=>$model->status==0?'Chưa chuyển kho':'Đã chuyển kho'),
		 array('name'=>'create_uid','value'=>$model->getCreateName()),
		array('name'=>'create_date','value'=>date("d-m-Y",strtotime($model->create_date))),
		 array('name'=>'update_uid','value'=>$model->getCreateName()),
         array('name'=>'update_date','value'=>date("d-m-Y",strtotime($model->update_date))),
