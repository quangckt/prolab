...
class ProoutController extends BaseController
{
	...
	public function actionIndex()
	{
                ...
                if(isset($_GET['Proout']))
		{
                       ...
                       if (!empty($model->dateout)) $criteria->addCondition('dateout = "'.$model->dateout.'"');
                     
                    	
                       if (!empty($model->numout)) $criteria->addCondition('numout = "'.$model->numout.'"');
                     
                    	
                       if (!empty($model->note)) $criteria->addCondition('note = "'.$model->note.'"');
                     
                    	
                       if (!empty($model->user1)) $criteria->addCondition('user1 = "'.$model->user1.'"');
                     
                    	
                       if (!empty($model->user2)) $criteria->addCondition('user2 = "'.$model->user2.'"');
                     
                    	
                       if (!empty($model->existbegin)) $criteria->addCondition('existbegin = "'.$model->existbegin.'"');
                     
                    	
                       if (!empty($model->used)) $criteria->addCondition('used = "'.$model->used.'"');
                     
                    	
                       if (!empty($model->existend)) $criteria->addCondition('existend = "'.$model->existend.'"');
                     
                    	
                       if (!empty($model->pm_print_id)) $criteria->addCondition('pm_print_id = "'.$model->pm_print_id.'"');
                     
                    	
                       if (!empty($model->pm_printplan_id)) $criteria->addCondition('pm_printplan_id = "'.$model->pm_printplan_id.'"');
                     
                    	
                       if (!empty($model->product_item_id)) $criteria->addCondition('product_item_id = "'.$model->product_item_id.'"');
