 ...
 * @property integer $inv_whouttype_id
 * @property integer $inv_wh_id
 */
class Proout extends BaseModel {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('id, user1, user2, pm_print_id, pm_printplan_id, product_item_id, inv_whouttype_id, inv_wh_id', 'numerical', 'integerOnly' => true),
            array('numout', 'length', 'max' => 255),
            array('note', 'length', 'max' => 500),
            array('existbegin, used, existend', 'length', 'max' => 19),
            // The following rule is used by search().
            // Please remove those attributes that should not be searched.
            array('id, dateout, numout, note, user1, user2, existbegin, used, existend, pm_print_id, pm_printplan_id, product_item_id, inv_whouttype_id, inv_wh_id', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function attributeLabels() {
        return array(
            ...
            'inv_whouttype_id' => 'Inv Whouttype',
            'inv_wh_id' => 'Inv Wh'
        );
    }
    ...
    public function search() {
        ...
        $criteria->compare('inv_whouttype_id', $this->inv_whouttype_id);
        $criteria->compare('inv_wh_id', $this->inv_wh_id);
    ...
    public function getUserInfo($userLoged, $userNameLoged) {
        $emp = Employee::model()->find('number=:number and status=0', array(':number' => $userLoged));
        if (!empty($emp)) {
            return $userNameLoged . ' (' . $emp->fullName . ')';
        } else {
            return 'Không có nhân viên nào ứng với user đang đăng nhập!!!';
        }
    }
